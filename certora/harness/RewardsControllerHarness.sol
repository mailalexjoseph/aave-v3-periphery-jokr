...
contract RewardsControllerHarness is RewardsController {
    ...
    function getAllUserRewards(address user, address asset, address reward)
        external
        view
        returns (uint256)
    {
        RewardsDataTypes.UserAssetBalance[] memory userAssetBalances = new RewardsDataTypes.UserAssetBalance[](1);
        userAssetBalances[0].asset = asset;
        (userAssetBalances[0].userBalance, userAssetBalances[0].totalSupply) = IScaledBalanceToken(
            asset
        ).getScaledUserBalanceAndSupply(user);

        return _assets[asset].rewards[reward].usersData[user].accrued + _getPendingRewards(user, reward, userAssetBalances[0]);
    }
    ...
    function getUserAccruedRewardsForAsset(address user,address asset, address reward)
        external
        view
        returns (uint256)
    {
    ...
    function getRewards(address user, address asset,address reward,uint256 userBalance,uint256 newAssetIndex)
        external
        view 
        returns(uint256)
    {
